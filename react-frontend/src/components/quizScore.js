...
const Score = ({ quiz, answersGiven, setAnswersGiven }) => {
  ...
  return (
    <div className="container-score" style={!hidden ? {justifyContent: "center"} : null}>
      ...
      <button type="button" onClick={() => setRedirect(true)} className="play-again-button" style={!hidden ? {position: "absolute", bottom: 0} : null}>Play again?</button>
    </div>
  );
}
